...
public class XomUtils {
	...
	public static Document getDocument(File xmlFile, boolean testValidity) {
		String xmlContents = IoUtils.read(xmlFile);
	...
	public static Document getDocument(String xmlContents, boolean testValidity) {
		...
		try {
			...
			Log.info("Validity test not passed when parsing XML string.");
			doc = ex.getDocument();
		} catch (ParsingException ex) {
			Log.warn("Parsing Exception:" + ex, ex);
		} catch (IOException ex) {
			Log.warn("IOException:" + ex, ex);
		}
